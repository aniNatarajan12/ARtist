...
extension SCNNode {
    ...
    func buildLineInTwoPointsWithRotation(from startPoint: SCNVector3, to endPoint: SCNVector3, radius: CGFloat, color: UIColor) -> SCNNode {
        ...
        let q0 = Float(0.0) //cos(θ/2)
        let q1 = Float(av_normalized.x) // x' * sin(θ/2)
        let q2 = Float(av_normalized.y) // y' * sin(θ/2)
        let q3 = Float(av_normalized.z) // z' * sin(θ/2)
